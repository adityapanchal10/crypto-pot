                                        ...
                                        <div class="row">
                                            <div class="col-md-6" id="tansactionSection">
                                                <div class="card card-tasks">
                                                    ...
                                                    <div class="card-header h">
                                                                <h4 class="card-title show" id="transactionH">You\'re trading '.$amount.' '.$from_wallet.' to '.$purchase_amount.' '.$to_wallet.'. Please confirm:</h4>
                                                            </div>
                                                            <div class="card-header">
                                                                <p class="card-category">Remaining balance in '.$from_wallet.' wallet: '.$from_wallet_balance.' '.$from_wallet.'.<br />
                                                                New balance in '.$to_wallet.' wallet: '.$to_wallet_balance.' '.$to_wallet.'.</p>
                                                            </div>
                                                    <div class="t">
                                                        <form class="dasht show" action="trade.php" method="POST" id="transaction">
                                                            <div class="input-wrapper">
                                                                <select id="inputFrom" class="form-control" name="from-wallet" disabled>
                                                                    <option selected>'.$from_wallet.'</option>
                                                                </select>
                                                                <label for="inputFrom">From</label>
                                                            </div>
                                                            <div class="input-wrapper">
                                                                <select id="inputTo" class="form-control" name="to-wallet" disabled>
                                                                    <option selected>'.$to_wallet.'</option>
                                                                    <!-- <option>BTC</option>
                                                                    <option>ETH</option>
                                                                    <option>BNB</option>
                                                                    <option>USDT</option>
                                                            ...
                                                            <div class="input-wrapper">
                                                                    <input type="number" id="amount" class="transferTo" required name="amount" value="'.$amount.'" disabled>
                                                                    <label for="transferTo">Enter amount</label>
                                                            </div>
                                                            <a href="dashboard.php">Back to Dashboard</a>
                                                            <button class="submit-btn" type="submit" name="confirm">Confirm</button>
...
} else if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['confirm'])) {
    if ($stmt = $con->prepare('SELECT remaining_balance, isKYCverified FROM userMaster WHERE email_id = ?')) {
        $stmt->bind_param('s', $_SESSION['email']);
        $stmt->execute();
        // Store the result so we can check if the account exists in the database.
                            ...
                            <div class="card card-tasks">
                                ...
                                <div class="t">
                                    <form class="dasht show" action="trade.php" method="POST" id="transaction">
                                        ...
                                        <div class="input-wrapper">
                                            <input type="number" id="amount" class="transferTo" required name="amount">
                                            <label for="transferTo">Enter amount</label>
                                        </div>
