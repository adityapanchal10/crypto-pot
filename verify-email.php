...
if (!isset($_SESSION['email'])) {
    ...
    if ($stmt = $con->prepare('SELECT isVerified FROM userMaster WHERE email_id = ?')) {
        ...
        if ($stmt = $con->prepare('SELECT first_name  FROM userMaster WHERE email_id = ?')) {
            ...
            if ($stmt->num_rows > 0) {
                ...
                if ($stmt = $con->prepare('UPDATE userMaster SET email_verfication_code = ? WHERE email_id = ?')) {
                    ...
                    $stmt->bind_param('ss', $code, $_SESSION['email']);
                    // echo $code;
                    if($stmt->execute()){
                        ...
                        if($mail->Send()) {
                            ...
                            <div class="center">
                                ...
                                <div class="login-content">
                                    ...
                                    <form class="loginform l3" action="verify-email.php" method="POST">
                                    <img src="">
                            ...
                            exit('<script>'.$mail->ErrorInfo.'; window.location = "login.php";</script>');
